...
Para rodar uma linha comando, selecione os comandos desejados e aperte `Ctrl+Enter` ou clique no botão `Run`, na parte superior do painel. Caso o código seja escrito diretamente no console, basta pressionar `Enter`. Para verificar os últimos comandos enviados ao console, clique no painel e utilize a tecla `seta para cima`. Por padrão, quando o console exibe o símbolo `>`, ele pronto para uso, aguardando o envio de um novo comando. Quando o símbolo `+` é apresentado, ele está aguadando a conclusão de algum comando enviado para o console. Quando algum comando mais demorado está sendo executado, um símbolo vermelho, com a palavra `stop` aparece no canto superior esquerdo do console. Você pode interromper a execução do comando clicando nesse ícone.
