...
Suas principais vantagens são a **velocidade**, **reprodutibilidade** e **eficiência** na produção destes relatórios. Por exemplo, a produção dos relatórios para 31 Regiões Administrativas do DF, a partir dos dados da PDAD 2018, com feita em Rmarkdown, tornando sua produção mais rápida e padronizada.
...
Para rodar uma linha comando, selecione a(s) linha(s) desejadas e aperte `Ctrl+Enter` ou clique no botão `Run`, na parte superior do painel. Caso o código seja escrito diretamente no console, basta pressionar `Enter`. Para verificar os últimos comandos enviados ao console, clique no painel e utilize a tecla `seta para cima`. Por padrão, quando o console exibe o símbolo `>`, ele pronto para uso, aguardando o envio de um novo comando. Quando o símbolo `+` é apresentado, ele está aguadando a conclusão de algum comando enviado. Quando algum comando mais demorado está sendo executado, um ícone vermelho, com a palavra `STOP` aparece no canto superior esquerdo do painel console. Você pode interromper a execução do comando clicando nesse ícone.
...
Consulte o diretório ativo do R, com a função `getwd()`. Para facilitar, salve os arquivos do R neste diretório ou altere o diretório padrão para um de sua preferência, com a função `setwd()`. Observação: o R utiliza o padrão de enderaçamento do Linux, então será sempre necessário trocar a barra `\` pela barra `/` quando o caminho for indicado.
...
O argumento `skip=1` informam à função que queremos que a primeira linha do arquivo seja ignoradae, enquanto o argumento `sheet=2`  indica que queremos carregar a segunda planilha.
...
*Observação: Quando estamos trabalhando com um objeto do tipo `data.frame`, podemos acessar suar colunas com o símbolo `$` após o nome do objeto. Alternativamente, podemos acessar as colunas com o operados `[,]`, no qual o lado esquerdo da vígula indica as linhas a serem acessadas e o lado esquerdo as colunas.* 
</div>
...
Vez ou outra nos deparamos com bases de dados que contêm variáveis do tipo data. Dentro da família `tidyverse`, temos o pacote `lubridate`, que facilita muito a manipulação deste tipo de dado.Na PDAD 2018, temos a `datavisita`, que traz o dia em que as informações foram coletadas no domicílio. Vamos utilizar essa variável para manipular datas.

Vamos verificar como estão preencidas as primeiras linhas dessa coluna, com auxílio da função `head()`.
...
Antes, vamos criar uma variável com a RA renomeada, de modo a facilitar a visualização.
...
Vamos verificar o resultado localmente. Procure no diretório o arquivo que acabou de ser criado e abra-o. Assim, você pode compartilhar esse resultado com outros pesquisadores.
...
Nesse código, o aspecto mais importante é a opção `na.rm=F`, na qual informamos que os valores ausentes não devem ser desconsiderados do cálculo. Verifique o valor amostral médio gerado com a função `mean`, agora considerando apenas os casos válidos.
...
A área acima corresponde às áreas urbanas e às áreas rurais com características urbanas. Tal área abrange a moradia de mais de 97% da população do DF.
...
vars_relatorio %>%
  ...
  srvyr::summarise(n=survey_mean(na.rm = T,vartype = "ci")) %>%
  ggplot()+
  geom_bar(aes(x=esgotamento_caesb,y=n,fill=esgotamento_caesb),stat = "identity",width = 0.75,alpha=0.5) 
```
